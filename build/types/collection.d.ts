...
type PatchedMap<K, V> = Omit & MapReplacementMembers;

interface PatchedMapConstructor {
	new (): PatchedMap;
	new <K, V>(entries?: readonly (readonly [K, V])[] | null): PatchedMap;
	new <K, V>(iterable: Iterable): PatchedMap;
	readonly [Symbol.species]: PatchedMapConstructor;
	readonly prototype: PatchedMap;
}
...
declare class Collection<T> extends Map {
	 ...
	 */
	find<S extends T>(
		condition: (e: T, index: number, collection: Collection) => e is S
	): S | undefined;
	find(
		condition: (e: T, index: number, collection: Collection) => boolean
	): T | undefined;
	 ...
	 */
	map<M>(
		transformer: (entity: T, index: number, collection: Collection) => M
	): M[];
